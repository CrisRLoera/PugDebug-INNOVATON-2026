...
function MetricCard({ label, value, icon, color, sub }: {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5">
      ...
      <p className="text-3xl font-bold text-slate-900">{value.toLocaleString('es-MX')}</p>
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([getAdminMetrics(), getAdminUsers()])
      .then(([m, u]) => { setMetrics(m); setUsers(u); })
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1 text-sm">Métricas globales de PugGuardian.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <h2 className="font-semibold text-slate-900">Usuarios registrados</h2>
          <p className="text-sm text-slate-500 mt-0.5">{users.length} usuarios en total</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50 text-left">
                ...
                <th className="px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Persona protegida</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Registro</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide text-right">Análisis</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide text-right">Estafas</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <p className="text-slate-700">{u.protectedPerson.name}</p>
                    <p className="text-slate-400 text-xs mt-0.5">{u.protectedPerson.phone}</p>
                  </td>
                  <td className="px-5 py-4 text-slate-500">{formatDate(u.registeredAt)}</td>
                  <td className="px-5 py-4 text-right font-semibold text-slate-900">{u.totalAnalyses}</td>
                  <td className="px-5 py-4 text-right">
                    <span className={`font-semibold ${u.fraudsBlocked > 0 ? 'text-red-600' : 'text-slate-400'}`}>
                      {u.fraudsBlocked}
                    </span>
                  </td>
                </tr>
