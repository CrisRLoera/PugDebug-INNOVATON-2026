...
import { getCatalogs, createCatalogItem, updateCatalogItem, deleteCatalogItem } from '../../api/admin';
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
        ...
        <span className="text-xs text-slate-400 bg-slate-100 px-2 py-1 rounded-full">{items.length} elementos</span>
      </div>

      {/* Item list */}
      <div className="divide-y divide-slate-100">
        {items.length === 0 && (
          <p className="px-5 py-4 text-sm text-slate-400 italic">Sin elementos. Agrega el primero.</p>
        )}
        {items.map((item) => (
...
export function CatalogManager() {
  ...
  useEffect(() => {
    getCatalogs().then(setCatalogs).finally(() => setLoading(false));
  }, []);
  ...
  async function handleUpdate(key: CatalogKey, item: CatalogItem) {
    await updateCatalogItem(key, item);
    setCatalogs((prev) => ({
      ...prev,
  ...
  async function handleDelete(key: CatalogKey, id: string) {
    ...
    setCatalogs((prev) => ({ ...prev, [key]: prev[key].filter((i) => i.id !== id) }));
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1 text-sm">Administra las opciones que aparecen en los formularios de registro.</p>
      </div>

      <div className="space-y-5">
        {(Object.entries(CATALOG_LABELS) as [CatalogKey, string][]).map(([key, label]) => (
          <CatalogSection
            key={key}
            title={label}
            catalogKey={key}
